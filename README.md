...
- **硬件路径追踪照明**：阳光、天空和发光方块能够直接或间接照亮场景，材质之间会传递
  颜色和亮度；
- **轻量路径追踪**：使用 Lambert 材质、首透明面反射和直线滤色，以性能为主，保留阳光与
  首个主表面的方块光采样；
...
- **实时渲染器**：选择完整路径追踪或以性能为主的轻量路径追踪；
- **降噪与图像重建**：兼容的 RTX 显卡可使用 DLSS RR；不可用时会回退到 NRD + FSR；
- **重建质量预设**：默认“性能”。画面流畅时可以逐步提高，帧率不足时降低；
- **额外镜面反弹次数**：控制实时主表面前的 Delta 反射/折射链，范围 1–64，默认 12；
- **最小反弹次数**：控制实时常规路径开始俄罗斯轮盘赌前的轮数下限，范围 1–8，默认 2；
- **最大反弹次数**：控制常规传输上限，范围 1–64，默认 12；实时中低于最小值时采用最小值，离线模式直接使用该总上限；
- **底色饱和度补偿**：默认开启，减轻完整模型高光造成的底色偏淡；可按喜好关闭；
...
- 透明介质最多可靠嵌套两个非空气区域。更深嵌套、非封闭模型和无法判断内外的边界不保证
  正确吸收或折射；
- 完整实时在首透明面随机选择反射或透射计算照明并补偿权重，两路 guide 仍补齐；后续
  按完整闭包采样。运动、链溢出或无效 guide 回退真实可见接口，不完整求解任意折射链；
- 轻量模式仅首可见透明面可以反射，之后直线透过并滤色，且深层表面不采方块光 NEE；
...
构建成功后，发行 JAR 为 `build\libs\prime-<mod_version>.jar`，版本取自 `gradle.properties`。
环境检查、测试、开发客户端和着色器调试
...
- [场景资源生命周期](docs/场景资源生命周期.md)
- [地形几何显存压缩](docs/地形几何显存压缩设计.md)
...
- [轻量路径追踪](docs/轻量路径追踪.md)
- [完整实时 Wavefront 调度](docs/完整实时Wavefront调度.md)
- [采样与路径终止](docs/采样与路径终止.md)
- [灯光与大气采样](docs/灯光与大气采样.md)
...
- [路径追踪性能约束](docs/路径追踪性能约束.md)
- [测试与基准验证](docs/测试与基准验证.md)
