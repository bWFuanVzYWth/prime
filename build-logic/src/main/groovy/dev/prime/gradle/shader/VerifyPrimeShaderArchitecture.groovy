...
abstract class VerifyPrimeShaderArchitecture extends DefaultTask {
    ...
    void verify() {
        ...
        def atmosphereProducers = ['spectrum', 'parameters', 'sky_rows', 'optical_depth', 'source', 'integrate'].collect {
            pathKey(new File(shaderRoot, "model/atmosphere/${it}.slang"))
        }
        productionEntries.findAll { !it.name.endsWith('.compute.slang') }.each { entry ->
            if (atmosphereProducers.any { closures[pathKey(entry)].contains(it) }) {
                throw new GradleException("RT entry reaches atmosphere precomputation: ${entry}")
            }
        }
