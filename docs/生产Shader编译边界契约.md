...
- 小模块不自动等于良好边界。拆分应减少 entry 可见闭包、降低修改扇出、形成可并行编译
  单元，或通过专用函数与类型改善实际中间代码生成和 GPU 执行；后者可以发生在同一 entry
  内，不要求增加 queue 或 dispatch。
...
已知消费范围必须落实为窄临时数据类型、按字段读取的 accessor 和专用算法。不得先生成完整
surface/BSDF/section 聚合对象，再依赖 Slang 删除无用成员或证明内部逻辑不可能。物理 GPU
record 可以共享且保持稳定，消费端类型不必复刻存储结构。按明确类型进行手动单态化、有限
代码生成或函数拆分均可采用；通用函数的常量实参或泛型本身不能证明最终特化已经发生。
运行时才确定的分类在边界显式分派，共有数学仍来自单一窄叶。

这项要求包含用户报告的历史实测背景：手动拆函数曾使 SPIR-V 体积减少约 50%、帧率提升约
30%。该结果不作为当前变更的性能验收值，但足以否定“自动 DCE 会处理好”的设计前提。
每次实际特化仍须记录自身的产物和 GPU 证据。
