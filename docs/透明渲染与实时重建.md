...
当前实时队列以 FP16 保存 `etaScale`，存活路径按实际使用的量化概率重加权，数学期望仍可保持，
但量化会改变存活分布、方差和重建输入。这不是永久精度批准；后续 encoding 必须按
[渲染核心数据 IR](渲染核心数据IR.md)的 RR 分布与图像误差合同重新验收。
...
两条分支复用已有透明 resolve、两套当前 144 字节路径槽和现有图像，不增加独立 guide trace、
...
当前 ABI 实际有两套 144 字节路径槽、每像素 272 字节阶段暂存和 10 个物理队列索引
（40 字节）。detached-guide 与随后 staged-light 的临时区按 phase lifetime 复用。完整 Wavefront
backing 为 `600 B × 内部分辨率像素数` 加 112 字节命令区，4K 为 4,976,640,112 bytes，即约
4746.09 MiB；字段和尺寸只以 `shaders/abi.json` 为准。RG32F
