...
两条分支复用已有透明 resolve、两套当前 112 字节路径槽和现有图像，不增加独立 guide trace、
...
- reflection motion：RG32F 归一化 UV；
...
`4 × (minimumBounces - 1) + 14`，不随最大反弹数增长。
当前有七个 queue command，部分逻辑角色按 phase 复用队列身份；各队列按可达并发宽度使用
`N` 或 `2N` 容量，生命期不重叠的索引存储复用紧凑 backing。
当前 ABI 实际有两套 112 字节路径槽、每像素 264 字节阶段暂存和 9 个物理队列索引
（36 字节）。area guide 只保存两个 FP16 radiance triplet 和一个 oct-SNORM16x2 方向，共 16 字节；
...
复用，guide 完成时显式重新发布两个分支的零 receiver normal。命中发光在 direct 阶段消费该
normal，queued scatter 因而不再依赖反向面积光树。完整 Wavefront backing 为
`524 B × 内部分辨率像素数` 加 112 字节命令区，4K 为 4,346,265,712 bytes，即约
4144.92 MiB；相比上一版减少 20 B/像素，4K 减少约 158.20 MiB。字段和尺寸只以
`shaders/abi.json` 为准。RG32F
reflection motion 另比 RG16F 增加
