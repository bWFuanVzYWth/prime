...
两条分支复用已有透明 resolve、两套当前 128 字节路径槽和现有图像，不增加独立 guide trace、
...
当前 ABI 实际有两套 128 字节路径槽、每像素 272 字节阶段暂存和 10 个物理队列索引
（40 字节）。detached-guide 与随后 staged-light 的临时区按 phase lifetime 复用。完整 Wavefront
backing 为 `568 B × 内部分辨率像素数` 加 112 字节命令区，4K 为 4,711,219,312 bytes，即约
4492.97 MiB；字段和尺寸只以 `shaders/abi.json` 为准。RG32F
