...
2026-09-06 用户确认画质满足需求，固定首透明面单光照策略：条件透射与反射都有效时由
STBN 等概率选择一条计算后续照明，throughput 乘 2；未选分支仅补 guide。完整路径固定
按 N 个槽分配，重建信号保留两套，双光照开关已删除。详见[实现记录](透明单光照分支实验.md)。

双分支基线是面向实时积分与重建的工程决策，不是对完整物理介质传输的等价化简。主要
原因有三项：
...
2. **随机或棋盘格选择首面事件会让两路信号在少数画面中欠采样。** 两种事件指向不同半球、
   ...
   双分支基线同时生成两条照明 continuation。当前仍保留两套 guide/history，以额外的
   guide-only 遍历避免未选分支丢失几何身份；该组合已通过用户画质验收。
...
guide 在后续离散界面优先沿条件透射（TIR 时反射）累计 PSR；与照明事件相同时复用 trace，
分歧时转入仅 guide 遍历。未选首面分支也沿该规则补 guide。运动、链溢出、上限和非法状态
沿用既有回退规则。NEE 阴影不折射，只累计直线吸收。

```mermaid
flowchart TD
    A["相机射线首命中透明接口"] --> B["条件透射与反射；STBN 选一支照明或双支对照"]
    B --> T["透射信号与 guide"]
    B --> R["反射信号与 guide"]
    T --> LF["后续透明面：完整闭包随机单分支"]
    R --> LF
    LF --> M["guide 共用 trace 或独立补齐；失败沿用回退"]
    M --> S{"实时重建后端"}
...
逻辑分支索引保持透射 0、反射 1，分别路由辐射、guide 与 SER coherence hint。单光照模式
只有选中分支进入 transport 队列，并映射到每像素唯一的 112 B 完整槽；另一支的 guide 射线
与控制/PSR 共 68 B，复用其闲置的 100 B 表面槽。

“第一个”严格指相机主射线的首个可见面。主射线先命中不透明面后遇到的透明面不会分叉。
光滑首面的反射响应包含 `F`，折射响应使用互补权重 `1-F`；粗糙首面保留闭包自身的 Fresnel、
有限立体角响应和在对应半球内归一化的条件 PDF。当前对既有双支估计器做 Bernoulli 稀疏化，
仅后续 throughput 乘逆选择概率，原条件 PDF/MIS 保持不变；首面 NEE/emission 不乘 2。
后续再次命中透明材质时不会扩张路径树：实时路径从 compact OpenPBR 闭包随机
选择一个反射或折射事件。
只有实际折射事件更新介质栈。
...
透射事件把 `relativeEta²` 累积到 `etaScale`。离线与实时固定线性 RR：
`min(maxRGB(throughput) × etaScale, 1)`；平方根和首 guide 去调制的四档开关及实现已删除。不另设透明链 RR 深度。
当前实时与离线路径都逐位保存 f32 `etaScale`；它不再经过 FP16 量化，因此相邻 dispatch 不会
改变 RR 存活分布。任何后续窄化仍必须按[渲染核心数据 IR](渲染核心数据IR.md)的 RR 分布与图像
误差合同重新验收。
...
这是面向实时重建的分层估计器：首可见面使用条件分支与可选 Bernoulli 稀疏化，后续顶点使用完整闭包
...
首面 guide，仅补出射首段距离。固定无 RR 前缀保证选中路径发布 guide 后再进入 tail RR。
...
Reflection motion 固定不生成、不提交。此策略经用户实测改善，已删除原粗糙度阈值分支、
投影逻辑与 RG32F 反射 MV 图像。主 scratch 仍用 R32F 距离的符号编码上一帧位置有效性，
...
每条有效 NRD 分支只在当前像素执行一次 SG resolve。PSR 尽可能共用照明 trace，分歧及
未选首面分支需要仅 guide 追踪。按透明覆盖率、guide 追踪步数和 fallback 率观察成本，
不能把只少一条照明路径等同于透明追踪成本减半。

仅选中的照明 continuation 进入 Wavefront 前缀。前缀轮数可在 1–8 之间调整，包含 landing
的主表面轮；调度数固定为 `4 × (minimumBounces - 1) + 15`，不随最大反弹数增长。未选
guide 在 split 后、照明 delta walk 前完成，避免每像素 detached 暂存冲突。
当前有七个 queue command，部分逻辑角色按 phase 复用队列身份；各队列按可达并发宽度使用
`N` 或 `2N` 容量，生命期不重叠的索引存储复用紧凑 backing。
完整路径槽固定为每像素一套 112 字节，其余仍为每像素 280 字节阶段暂存和 9 个物理队列索引
（36 字节）。每像素 guide 共 32 字节：前 16 字节保存两个 FP16 area radiance triplet 和一个
oct-SNORM16x2 方向；后 16 字节保存透明可见接口的 f32 normal/roughness，普通像素不使用。
旧 16 字节 stride 会让接口数据覆盖下一像素，现已修复；
guide queue 在初始 area queue 消费后复用其第一个索引槽，并在 tail admission 再次使用 area queue
前结束。24 字节/路径的后期暂存前 12 字节保存 light selection，后 12 字节逐位保存反向
MIS 所需的 previous receiver normal；它整体与更早的 40 字节 detached-guide 按 phase lifetime
复用。两条分支的阶段记录现在并排位于各像素的 48 B 区域，guide 占同像素前 40 B，修正
旧紧密 guide / branch-major normal 排布的跨像素覆盖；guide 完成时显式重新发布本像素
两个分支的零 receiver normal。命中发光在 direct 阶段消费该
normal，queued scatter 因而不再依赖反向面积光树。历史双光照 Wavefront backing 为
`540 B × N + 112 B`；单光照为 `428 B × N + 112 B`，另有不足 256 B 的路径区对齐。
4K 分别为 4,478,976,112 与 3,550,003,312 bytes，省 885.9375 MiB。单光照的未选 guide
不使用上述 phase 区，而在其闲置表面槽中保留 40 B 射线 + 28 B 控制/PSR 至 resolve。
32 B guide 步长修复继续保留。字段和尺寸以
`shaders/abi.json` 为准。DLSS 删除不再提交的 RG32F 反射 MV，额外省下 `8 B × N`，4K 为
63.28 MiB。R16F specular hit distance 复用 sun penumbra 图像，继续保留。
普通路径 previous position 复用现有 PSR cold lane，不再增加 backing 或
逐 bounce 图像写带宽。
...
只推进一条辐射 continuation，另一支仅补 guide；非透射像素不探测本应无效的反射分支。进一步优化必须证明
...
DLSS 仅提交 reflection hit distance 经用户实测改善，已收敛为固定策略，会话对照开关与未提交的 MV 图像已删除。
NRD 太阳 specular 纳入 REBLUR 导致严重高光阴影漏光，已撤回；普通表面的太阳 diffuse
和 specular 恢复共用 SIGMA。透明 specular albedo 来源修复和每像素 guide stride 修复
始终保留。具体信号、内存、验收和适用范围见
[重建输入对照实验](重建输入对照实验.md)。
