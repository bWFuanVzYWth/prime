...
当前 ABI 实际有两套 144 字节路径槽、每像素 320 字节 Area/阶段暂存和 10 个物理队列索引
（40 字节）。完整 Wavefront backing 为 `648 B × 内部分辨率像素数` 加 112 字节命令区，4K 为
5,374,771,312 bytes，即约 5125.78 MiB；字段和尺寸只以 `shaders/abi.json` 为准。RG32F
