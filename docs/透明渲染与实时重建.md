...
- 每个实体玻璃或水边界按自己的 RGB 消光累计带符号的一阶矩：进入边界加
  ...
  几格水程所产生的 RGB 浮点消去；
- 当前 Wavefront 记录逐位保留起点介质的 f32 RGB 消光与 IOR，避免相邻 dispatch 改变
  Beer-Lambert 衰减或在临界角两侧选择不同的 TIR 事件；这是保守参数 encoding。阴影边界与
  起点消光目前仍共同规范化到 fp16 识别域，这是待迁移问题：介质身份必须改由精确
  `MediumId`/边界关系表达，连续消光只参与数值计算。匹配后不再做 RGB 浮点相减，只通过绕数抵消；因此队列
