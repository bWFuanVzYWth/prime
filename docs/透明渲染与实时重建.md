...
当前 ABI 实际有两套 112 字节路径槽、每像素 280 字节阶段暂存和 10 个物理队列索引
（40 字节）。24 字节/路径的后期暂存前 12 字节保存 light selection，后 12 字节逐位保存反向
...
`544 B × 内部分辨率像素数` 加 112 字节命令区，4K 为 4,512,153,712 bytes，即约
4303.13 MiB；相比上一版减少 24 B/像素，4K 减少约 189.84 MiB。字段和尺寸只以
