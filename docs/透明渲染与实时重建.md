...
- shadow 与主路径读取同一 relation。薄壁事件不改变当前介质；实体 substrate 的真实折射才
  改变介质端点。
...
事件，只有实际透射更新当前介质。首面条件估计器的单分支化保持逆概率重加权，不修改
紧凑 OpenPBR 数学。

选中分支映射到每像素唯一的 96 B 完整路径槽。未选分支用闲置的 100 B 表面槽保存
36 B 精确 guide 射线与 28 B 控制/PSR；它在选中照明的 delta walk 前排空，避免临时区
...
- 水使用实测纯水 RGB 消光。只有采到实体介质的折射事件才在进入/退出时更新当前介质；玻璃
  与水都保存各自的 IOR 和消光。
...
- surface relation 必须在构造 BSDF 前解析为单一 `SurfaceInteraction`。overlay 的 alpha
  只能选择 overlay 或 substrate；只有选中的 substrate 透射才进入本节的当前介质逻辑。
...
NEE 阴影 payload 携带 RGB 光学厚度矩、终点介质消光、一个起点介质的精确
`MediumId` / 消光、最远已处理边界距离和真实遮挡距离：

- 起点介质同时初始化终点消光，不预乘 `tMax`；没有边界时直接得到完整起始介质段；
- 每个实体边界提供两侧介质。沿射线从 A 穿到 B 时累计 `(sigmaA - sigmaB) × t` 的
  带符号距离矩，实现中分别累加两项以保持现有求值顺序；不把嵌套区域的消光相加；
- 终点消光取距离最远边界的目标介质，通过距离归约决定，不依赖 any-hit 调用顺序。
  最终光学厚度为 `moment + endpointExtinction × tMax`。射线已离开全部实体时，末端
  消光直接为零，避免浮点消光加减留下残差后再被太阳的百万单位 `tMax` 放大；
- 当前 Wavefront 记录逐位保留 f32 RGB 消光，以 exact Fresnel source / water code
  重建 IOR。阴影边界只按 renderer-lifetime `MediumId` 匹配起点介质，匹配时使用同一
  起点消光。阴影积分继续沿用既有 fp16 round-trip；这不改变路径记录的 f32 encoding；
...
- 翻译层必须发布唯一、方向明确的物理边界；不允许依靠下游猜测缺失端点、恢复历史或
  掩盖相互矛盾的介质。末端非负钳制仅处理距离矩浮点舍入；
...
并累计完整 f32 路径长度和反射旋转。照明的随机事件相同时共用追踪；事件分歧时，以 36 B
...
两条逻辑分支复用已有透明 resolve 和图像。单光照模式仅有一份 96 B 运输记录，未选分支
...
Rec.2020 到显示 sRGB 的 RGB Reinhard 变换。
...
| 层级 | 主要检查 |
| --- | --- |
| `PrimeBsdfGpuTest` / `PrimeBsdfDiagnosticsGpuTest` | 首面光滑 `(1-F)/η²` 折射、粗糙条件反射/透射 sample/eval/PDF、后续完整闭包、TIR、实体玻璃/水当前介质，以及拒绝/净化前的 NaN/Inf、负值和非法方向诊断 |
...
完整路径每像素 96 B，另有 280 B 阶段暂存、九个物理 uint 索引和全帧七条 16 B command。
总 backing 为 `align256(96N)+316N+112` bytes。逻辑队列按可达并发宽度分配 N 或 2N
...
- 每像素 48 B phase 区先保存 36 B detached-guide，随后保存两份 24 B 分支暂存：前
...
- 未选首面 guide 使用自身闲置表面槽，36 B 射线与 28 B 控制/PSR 不占上述 phase 区。
