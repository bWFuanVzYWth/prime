...
下面两轮数据属于已替换的减法补偿，不能作为固定幂函数的误差或性能验收。尤其是第二轮
验证只覆盖同一颜色内部的通道关系；用户随后发现跨颜色反序，这是最小通道影响其他通道
所导致的映射性质，不是连续性测试可以排除的问题。

2026-09-06 首版验证（逐通道钳制）：15,015 组介电参数、目标颜色 `(0.24, 0.46, 0.78)` 的正视总能量
最大通道误差为 `0.002823`，各通道误差均低于未补偿时的 15%。83 个生产 artifact 中，
62 个逐字节不变（包括 Lambert 与全部 any-hit）；21 个消费者的非调试函数体指令数增加
`0.27%–1.22%`，fixed/landing scatter 分别约 `0.56% / 0.54%`。这是 SPIR-V 静态规模，
不代表 GPU 帧时间或寄存器实测；本轮没有新的 Nsight 寄存器数据。路径 ABI 与 descriptor 门禁通过。

同日改为限制共享补偿量后，`build shaderTest` 通过：581 项 CPU、56 项 GPU、1 项产物测试，
无失败、错误或跳过。上述正常颜色的最大通道误差仍为 `0.002823`；另有 6,468 组暗色参数
检查通道排序、差值比例及 guide 一致性，257 个渐变点跨越补偿上限，未出现颜色跳变。
此连续性检查不宣称硬 `min` 具有连续导数。黑白与不参与补偿的拓扑继续通过原有检查。

相对逐通道钳制版本，83 个生产 artifact 中仍有 62 个逐字节不变；21 个消费者的非调试
函数体指令增加 `0.036%–0.114%`，fixed/landing scatter 各增加 40 条（约 `0.075% / 0.073%`）。
仅原有 11 个 entry 的源码预算各增加 138 B，没有新增依赖、查表、路径字段或 dispatch。
实测寄存器和帧时间仍待捕获，不能从这点静态增量推断。记录位于
`build/register-analysis/albedo-cap-report.txt`，构建日志为 `albedo-cap-build.log`。

固定 `pow(target, 1.04)` 与设置开关版本：`build shaderTest` 通过，584 项 CPU、56 项 GPU、
1 项产物测试均无失败、错误或跳过。15,015 组光学参数分别开启/关闭补偿，验证固定映射、
关闭后的源颜色及能量恢复、guide 一致性；另有 6,150 组跨颜色/光学参数渐变验证通道独立性
与逐通道单调性。配置测试覆盖旧文件默认开启、显式关闭持久化、非法值回退和历史失效。

相对限制共享减法补偿的版本，21 个实际消费者的非调试函数指令减少 `0.108%–0.488%`，
fixed/landing scatter 各减少 120 条（约 `0.225% / 0.219%`）。其余 62 个 artifact 的函数
指令数不变，包括 Lambert 与全部 any-hit；ABI 声明拆分也改变了产物布局，只有 15 个
artifact 逐字节不变。幂函数的实际执行成本与寄存器仍需新捕获，不能由 SPIR-V 条数推断。
窄 push 声明与重导出沿用同一绑定，全部生产 depfile、ABI、打包与星图完整性检查通过。
统计与构建日志见 `build/register-analysis/albedo-power-report.txt` 和
`build/register-analysis/albedo-power-verified-build.log`。
